...
def Add_bathymetry(fig,ax, colorbar = True):
    """Adds Bathymetry to the given ax, provide plt fig and ax, 
    Filepath: Location of Bathymetry,"""
    from matplotlib import cm
    import functions.settings as settings
    if not settings.BATHYMETRY_FILE.is_file():
        print('BATHYMERTRY FILE does not exit did not plot bathymetry')
        return fig, ax
    bath = xr.open_dataset(settings.BATHYMETRY_FILE)
