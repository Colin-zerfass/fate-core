...
if __name__ == '__main__':
    ...
    for name, uo_col in models.items():
        ...
        anoms = longlist[uo_col] - np.mean(longlist[uo_col])
        longlist['Uz_' + suffix] = Z *anoms+  np.mean(longlist[uo_col])
    ...
    print('--- GLORYS - STOKES Regression---')
    coef = Regression(longlist, Uo = 'Uo_30', W = 'Ustokes')
    suffix = '_stokes'
    longlist = regression_u(longlist, coef, Uo='Uo_30', W = 'Ustokes', suffix=suffix)
    print_coefficients(name, coef)
    print_contributions(name, coef, np.mean(np.abs(longlist['Uo_30'])), np.mean(np.abs(longlist['Ustokes'])))
    R_raw = calc_R_anything(longlist.U, longlist['Uo_30'])
    R_reg = calc_R_anything(longlist.U, longlist['Ureg_' + suffix])
    print(f'  R (raw): |R|={np.abs(R_raw):.3f}, angle={np.angle(R_raw, deg=True):.1f} deg')
    print(f'  R (reg): |R|={np.abs(R_reg):.3f}, angle={np.angle(R_reg, deg=True):.1f} deg')
    print()
