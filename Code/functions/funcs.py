...
def Palmyra_plot(ax):
    Palmyra = Palmyra_obj()
    ax.scatter(Palmyra.x,Palmyra.y, marker  = "*", color = "r")
    return ax 
...
def add_distance_collumns(data):
    ...
    for n in range(0,length):
        ...
        for n in range(0,len(x_coords)-1): ##this has to be one less because using n+1
            m = int(n+1)
            xdist = haversine_dist(x_coords[n], y_coords[n], x_coords[m],y_coords[n])## gives just the x distance traveled
            ydist = haversine_dist(x_coords[n], y_coords[n], x_coords[n],y_coords[m]) ## gives just the y distance traveled
...
def interp_currents(data,vo,uo):
...
def Rolling_mean(x = list,windowsize = int):
    ...
    line = line["speed"].rolling(windowsize, center=True).mean()
    line  = line.to_list()
    return line
