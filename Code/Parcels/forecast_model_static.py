...
def Run_model_static(startmonth:pd.Timestamp, endmonth:pd.Timestamp, monthindex:int, configfile:str):
    import parcels
    ...
    #def Run_model(startmonth:pd.Timestamp, endmonth:pd.Timestamp, monthindex:int, configfile:str, log_lock=None): 
    ...
    ds_timerange = querry_date_range(ds,startmonth , endmonth).reset_index()
    ...
    engine = Alligner(ds)
    engine.allign_data_MultipleDays(combined_ds, startmonth, endmonth,
    ...
    # with log_lock if log_lock is not None else open(os.devnull):
    #     with open(r"output\Output_logs.txt", "a") as log_file:
    #         log_file.write(f"Model {monthindex} {startmonth} {endmonth} run complete\n")
